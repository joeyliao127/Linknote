...
### 1. 啟動基礎服務（postgres / redis / qdrant）
...
```bash
cd Docker/build/springboot
docker compose -f docker-compose-springboot.yaml up -d

cd ../fastapi
docker compose -f docker-compose-fastapi.yaml up -d

cd ../nuxt
docker compose -f docker-compose-nuxt.yaml up -d
